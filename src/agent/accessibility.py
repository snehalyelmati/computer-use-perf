...
def _role_to_abbr(role: str) -> str:
    ...
    mapping = {
        'button': 'btn', 'link': 'link', 'textbox': 'inp', 'searchbox': 'inp',
        'combobox': 'sel', 'listbox': 'sel', 'checkbox': 'checkbox',
        'radio': 'radio', 'switch': 'switch', 'slider': 'slider',
        'tab': 'tab', 'canvas': 'canvas', 'option': 'option',
        'menuitem': 'menu', 'menuitemcheckbox': 'menu', 'menuitemradio': 'menu',
        'treeitem': 'tree', 'spinbutton': 'inp',
        'disclosuretriangle': 'details',  # <details>/<summary>
    }
    return mapping.get(role.lower(), role[:6] if role else '?')
...
def _build_ancestor_labels(ax_nodes: list[dict], interactive_node_ids: set) -> dict:
    ...
    CONTAINER_ROLES = {'form', 'dialog', 'group', 'region', 'list', 'listbox',
                       'menu', 'menubar', 'navigation', 'main', 'complementary',
                       'contentinfo', 'banner', 'radiogroup', 'tablist', 'toolbar'}
        ...
        while current:
            ...
            if parent:
                ...
                if role in CONTAINER_ROLES and name:
                    ancestors.append(f"{role}:{name[:20]}")
...
def format_full_a11y_tree(ax_nodes: list[dict], interactive_indices: set[str]) -> str:
    ...
    for node in ax_nodes:
        ...
        if role in CONTAINER_ROLES and name:
            containers.append(f"{role}: {name[:40]}")
            continue
    ...
    if code_candidates:
        ...
        for code in code_candidates[:10]:
            lines.append(f"- **{code}**")
    ...
    if other_text:
        ...
        seen = set()
        for t in other_text:
            if t not in seen and len(seen) < 30:
                seen.add(t)
                lines.append(f"- {t[:60]}")
...
async def extract_from_accessibility_tree(page: Page) -> tuple[list[dict], list, str]:
    ...
    try:
        ...
        interactive_node_ids = {n.get("nodeId") for n in interactive_nodes}
        ...
        for i, node in enumerate(interactive_nodes):
            ...
            try:
                result = await cdp.send("DOM.resolveNode", {"backendNodeId": backend_id})
        ...
        indexed_node_ids = {n.get("nodeId") for n in interactive_nodes}
